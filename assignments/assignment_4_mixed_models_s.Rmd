...
tidy(painAhosp_mdl, conf.int = TRUE)|>
...
```{r}
pred_painB <- painB |> mutate( pred_pain = predict(painA_hospmd, painB, allow.new.levels = TRUE))
...
# R2 dataset B
1- (sum((pred_painB$pain-pred_painB$pred_pain)^2) / sum((pred_painB$pain-mean(pred_painB$pain))^2))
R2(pred_painB$pred_pain, pred_painB$pain)
```

# Discussion
With the inclusion of the hospital clustering the effect of sex, pain categorising and trait anxiety dropped below significance level, while the other predictors effect increased. The confidence intervals are seemingly narrower in the previous model. The newer model considers the effect of the hospitals.
